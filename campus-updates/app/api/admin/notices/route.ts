...
const extractTimeSent = (text: string): string | undefined => {
	...
	const isoMatch = text.match(
		/(?:📅|⏰|Start:|Sent:|On:)\s*([0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(?:\.[0-9]+)?(?:Z|[+-][0-9]{2}:?[0-9]{2})?)/i,
	);
	...
	const humanMatch = text.match(
		/\*?On:?\*?\s*:?\s*([A-Za-z]+)\s+(\d{1,2}),?\s+(\d{4})\s+at\s+(\d{1,2}):(\d{2})\s*(AM|PM)\s*(?:IST)?/i,
	);
...
const processNotice = (notice: any) => {
	...
	const extractedTime =
		extractTimeSent(processed.formatted_message) ||
		extractTimeSent(processed.content);
...
export async function GET() {
	...
	try {
		...
		const normalizedData = data.map(processNotice).filter((n: any) => {
			...
			return NextResponse.json(
				{
					ok: false,
					error:
						"Notice must have at least title, content or formatted_message",
				},
				{ status: 400 },
			);
		}
