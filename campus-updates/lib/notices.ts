...
export const normalizeCategory = (cat: string): string => {
	...
	if (c === "internship_noc") return "internship noc";
	return c;
};
...
export const isPlacementBotPost = (
	notice:
		| Partial<Notice>
		| {
				formatted_message?: string;
	...
	if (author && /bot|placementbot|placement-bot|placement_bot/i.test(author))
		return true;
	...
	const isShort =
		(text || "").trim().length > 0 &&
		(text || "").trim().length < lengthThreshold;
